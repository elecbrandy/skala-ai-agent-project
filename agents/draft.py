...
- **TRL 추정 테이블** (각 기업별):
  ...
  |------|----------|--------|------------------|
  | ...  | TRL X    | HIGH/MED/LOW | 간접 지표·공급 발표 등 요지 [R2][W1] |

  ⚠ "근거 요약" 칸에는 반드시 아래 context에 주어진 [R#]/[W#] 인용 번호를
  1개 이상 포함해야 합니다. 인용 없는 TRL 행은 금지됩니다.
...
## REFERENCE
- **본문에서 인용한 [R#]/[W#] 번호를 그대로 사용**하여 나열하세요.
- 본문에서 한 번도 인용되지 않은 출처는 REFERENCE 에 포함하지 마세요.
- 형식:
  - [R#] "파일명", 내부 문서, [신뢰도: HIGH/MED/LOW]
  - [W#] "제목", 출처명, 날짜, URL, [신뢰도: HIGH/MED/LOW]
- 신뢰도 기준:
...
필수 준수사항
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. 모든 TRL 추정 행의 "근거 요약" 칸에 반드시 [R#]/[W#] 인용 번호를 1개 이상 넣으세요.
   인용 없는 TRL 추정은 금지됩니다. context 에 주어진 번호만 사용하고, 새로 만들지 마세요.
2. 3. 경쟁사 동향의 간접 지표(특허·학회·채용·공급) 서술에도 가능한 한 [R#]/[W#] 을 함께 표기하세요.
3. TRL 4~6 추정 시 반드시 한계 고지 문구를 포함하세요 (위 2섹션 참조).
4. 신뢰도 레이블(HIGH/MED/LOW)을 모든 TRL 추정치 옆에 표기하세요.
5. 긍정(기술 진전)·부정(한계·위험)·중립(간접 지표) 관점을 균형 있게 서술하세요.
6. 수정 피드백이 있다면 해당 섹션을 구체적으로 보강하세요.
7. 추측성 서술은 "~으로 추정", "~가능성 있음" 등 추정 표현으로 명확히 구분하세요.
"""
...
def draft_node(state: AgentState) -> dict:
    ...
    rag_text = "\n".join(
        f"[R{i}] source={r['source']} date={r['date']}\n  {r['content'][:300]}"
        for i, r in enumerate(rag[:10], start=1)
    )
    web_text = "\n".join(
        f"[W{i}] bias={r['bias_label']} date={r['date']} url={r['url']}\n  {r['content'][:200]}"
        for i, r in enumerate(web[:10], start=1)
    )
